    ...
    for (fsh in fshs){
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                plotZScores(afits,label=paste(fsh,": ","retained catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("Plotting retained catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScores(afits,label=paste(fsh,": ","retained catch biomass",sep=''));
            }
            if (!is.null(fit$n.at.z)){
                cat("Plotting retained catch size frequency zscores for",fsh,"\n")
                plotZScoresForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","retained catch",sep=''))
                cat("Plotting ESSs for size frequencies.\n")
                plotEffNsForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","retained catch",sep=''))
            }
        }
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                afits<-fit$abundance$fits;
                plotZScores(afits,label=paste(fsh,": ","discard catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("PLotting discard catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScores(afits,label=paste(fsh,": ","discard catch biomass",sep=''));
            }
            if (!is.null(fit$n.at.z)){
                cat("Plotting discard catch size frequency zscores for",fsh,"\n")
                plotZScoresForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","discard catch",sep=''))
                cat("Plotting ESSs for size frequencies.\n")
                plotEffNsForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","discard catch",sep=''))
            }
        }
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                afits<-fit$abundance$fits;
                plotZScores(afits,label=paste(fsh,": ","total catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("Plotting total catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScores(afits,label=paste(fsh,": ","total catch biomass",sep=''));
            }
            if (!is.null(fit$n.at.z)){
                cat("Plotting total catch size frequencies for",fsh,"\n")
                plotZScoresForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","total catch",sep=''))
                cat("Plotting ESSs for size frequencies.\n")
                plotEffNsForSizeFreqs(fit$n.at.z,res$mc,label=paste(fsh,": ","total catch",sep=''))
            }
        }
    }
}
