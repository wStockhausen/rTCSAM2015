    ...
    for (fsh in fshs){
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                plotZScoresGG(afits,label=paste(fsh,": ","retained catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("Plotting retained catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScoresGG(afits,label=paste(fsh,": ","retained catch biomass",sep=''));
            }
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                afits<-fit$abundance$fits;
                plotZScoresGG(afits,label=paste(fsh,": ","discard catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("PLotting discard catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScoresGG(afits,label=paste(fsh,": ","discard catch biomass",sep=''));
            }
            if (!is.null(fit$n.at.z)){
        ...
        if (!is.null(fit)){
            if (!is.null(fit$abundance)){
                ...
                afits<-fit$abundance$fits;
                plotZScoresGG(afits,label=paste(fsh,": ","total catch abundance",sep=''));
            }
            if (!is.null(fit$biomass)){
                cat("Plotting total catch biomass zscores for",fsh,"\n")
                afits<-fit$biomass$fits;
                plotZScoresGG(afits,label=paste(fsh,": ","total catch biomass",sep=''));
            }
            if (!is.null(fit$n.at.z)){
