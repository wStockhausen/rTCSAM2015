#'
#'@title Get model fits to abundance, biomass or size frequencies as z-scores for fleet data components
#'
#'@title Function to get model fits to abundance, biomass or size frequencies as z-scores for fleet data components.
#'
...
#'@return dataframe
#'
#'@details Uses \code{getMDFR.ZScoresForABData()} and \code{getMDFR.ZScoresForSizeComps()}.
...
#'@export
#'
getMDFR.ZScoresForFleets<-function(repObjs,
                                   fleet.type='fishery',
                                   data.type='abundance',
                                   verbose=FALSE){
    if (verbose) cat("--Starting getMDFR.ZScoresForFleets.\n");
    mdfr<-NULL;
    if (!inherits(repObjs,'tcsam2015.rep')){
        #repObjs should be a list of tcsam2015 model report objects
        for (nm in names(repObjs)){
            mdfrp<-getMDFR.ZScoresForFleets(repObjs[[nm]],fleet.type=fleet.type,
                ...
                for (ctNm in ctNms){
                    ...
                    if (!is.null(ct)){
                        ...
                        if ((data.type=="n.at.z")&&!is.null(ct$n.at.z)){
                            ...
                            mdfrp<-getMDFR.ZScoresForSizeComps(ct$n.at.z,repObjs$mc,verbose=verbose);
    ...
    if (verbose) cat("--Finished getMDFR.ZScoresForFleets.\n");
    return(mdfr);
}
